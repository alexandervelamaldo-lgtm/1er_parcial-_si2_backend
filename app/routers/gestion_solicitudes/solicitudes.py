...
async def list_requests(
    ...
    if diagnostico is not None and try_parse_categoria_dano(diagnostico) is None:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Filtro de diagnóstico inválido. Usa una categoría autorizada.",
        )
    diagnostico_normalizado = (
        try_parse_categoria_dano(diagnostico).value if diagnostico is not None else None
    )
    query = (
        select(Solicitud)
    ...
    async with _open_solicitud_session(
        ...
        if auto_aceptada:
            ...
            db.add(
                HistorialEvento(
                    solicitud_id=solicitud.id,
                    estado_anterior=propuesta_state.nombre,
                    estado_nuevo=estado_asignada.nombre,
                    observacion=(
